...
################################ Else ################################
def AV_path_save_in_vattha(rel_path_p, FN_l):
	"""
...
def AV_pandas_append_df_to_excel(filename, df, sheet_name='Sheet1', startrow=None, truncate_sheet=False, **to_excel_kwargs):
    """
...
def AV_spss_read(FN_p):
    ...
    df_l, meta_l = pyreadstat.read_sav(FN_p)

    column_names_l = meta_l.column_names # ID of columns.
    column_names_to_labels_l = meta_l.column_names_to_labels # A dictionary: Keys are column_names_l and Values are corresponding description of columns.
    variable_value_labels_l = meta_l.variable_value_labels # A dictionary of dictionaries: Keys are column_names_l and Values are a dictionary, of which key (e.g. 1.0) is a score and value is a text "Totally Disagree".

    df_l.columns = column_names_l

    # print(meta_l.column_names)
    # print(meta_l.column_labels)
    # print(meta_l.column_names_to_labels)
    # print(meta_l.value_labels)
    # print(meta_l.number_columns)
    # print(meta_l.file_label)
    # print(meta_l.file_encoding)
...
################################ Various plots ################################
def AV_plot_line_ConfInt(axe_p, x_p, y_p, yerr_p, label_p=None, color_p="b", linewidth_p=5, linestyle_p="-", marker_p=None, markersize_p=5, alpha_p=0.2):
	"""
	yerr_p should be positive.
	"""

	axe_p.plot(x_p, y_p, color=color_p, linewidth=linewidth_p, linestyle=linestyle_p, marker=marker_p, label=label_p, markersize=markersize_p)
	axe_p.fill_between(x_p, y_p - yerr_p, y_p + yerr_p, alpha=alpha_p, color=color_p)


def AV_plot_line_errorbar(axe_p, x_p, y_p, yerr_p, xerr_p, color_p=None, bar_color_p=None, label_p=None, capsize_length_p=5, capsize_thick_p=3, linewidth_p=5, alpha_p=0.5):
